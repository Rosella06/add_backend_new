...
class PickupService {
  ...
  public async initiatePickup(
    ...
    if (this.isSlotBusy(machineId, slot)) {
      throw new HttpError(409, `Pickup process for slot ${slot} on machine ${machineId} is already in progress.`);
    }
    ...
    logger.info(TAG, `Slot '${slot}' on machine ${machineId} is now BUSY for pickup.`);
  ...
  private startPickupCompletionLoop(
    ...
    if (!socket) {
      logger.error(TAG, `Cannot start pickup loop for ${orderId}, socket disconnected.`);
      this.busySlots.delete(slotIdentifier);
      return;
    }
    ...
    const intervalId = setInterval(async () => {
      ...
      try {
        ...
        const trayIsEmpty = await plcService.isTrayEmpty(socket, machineId, slot);

        logger.debug(TAG, `[Loop for ${orderId}] Status: Door Closed=${doorIsClosed}, Tray Empty=${trayIsEmpty}`);

        if (doorIsClosed && trayIsEmpty) {
          clearInterval(intervalId);
          this.busySlots.delete(slotIdentifier);
          logger.info(TAG, `Slot '${slot}' on machine ${machineId} is now FREE.`);
          ...
          logger.info(TAG, `Emitting PICKUP_COMPLETED for machine: ${machineId}`);
          systemEventEmitter.emit(SystemEvents.PICKUP_COMPLETED, { machineId });
        }
      } catch (error) {
        logger.error(TAG, `Error in pickup completion loop for ${orderId}:`, error);
      }
